    ...
    if re.match(config['alertKeywords'], subjectField, flags=0):
        ...
        if m == 1:
            ...
            if body:
                ...
                if "Update" in subjectField: #update code
                    ...
                    try:
                        ...
                        print("Unable to update, unable to find two '---'s, exiting..")
                        sys.exit(0)
            ...
            try:
                ...
                alert = Alert(title=subjectField,
                              tlp=int(config['alertTLP']),  #setting it blank since custom template allows default color, set it back to tlp = int for conf value
                              tags=caseTags,
                              description=body,
                              type='external',
                ...
                print("Error with creating alert, wrong template name or tags?")
        else:
            print("")
         # Create the alert
        response = api.create_alert(alert)
...
def updateACase(caseId, bigGroup, fromField, attachments):
    ...
    resolvedStatus = parseBody("Resolution Status", bigGroup) # (Indeterminate,FalsePositive, TruePositive, Other or Duplicated)
    ImpactStatus = parseBody("Impact Status", bigGroup) #(NoImpact, WithImpact or NotApplicable) -
    ...
    try:
        ...
        print("unable to find the caseId in the update email, this is required to uniquely identify a case to update. ending..")
        sys.exit(0)
    try:   
        ...
        correctResolutions = ["Indeterminate", "FalsePositive", "TruePositive", "Other", "Duplicated"]
        ...
        if "yes" in resolved:
            print("resolved")
            resolveUpdate = api.case.update(requestCase,
                                           status='Resolved')                            
        if "no" in resolved:
            print("OPEN")
            resolveUpdate = api.case.update(requestCase,
